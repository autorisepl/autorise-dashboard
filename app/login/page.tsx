...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        const from = params.get('from') ?? '/agenci'
        router.push(from)
        router.refresh()
      } else {
      ...
      <div style={{ width: '100%', maxWidth: 360, padding: '0 20px' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#566a8a', marginBottom: 8, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              Hasło dostępu
            </label>
            ...
            {error && (
              <div style={{ fontSize: '12px', color: '#f87171', marginTop: 8, display: 'flex', alignItems: 'center', gap: 5 }}>
                {error}
              </div>
            )}
          </div>
