...
async function fetchKRSOdpis(krsNumber: string): Promise<{
  ...
  try {
    ...
    const pkdMain = dzial1?.pkdDzialalnosciPrzewazajacej ?? dzial1?.pkdPrzewazajaca ?? dzial1?.pkdGlowne
    if (pkdMain) {
    ...
    if (siedz?.adres) {
      adres = typeof siedz.adres === 'string' ? siedz.adres : formatAdres(siedz.adres as Record<string, unknown>)
    ...
    if (Array.isArray(czlonkowie)) {
      for (const c of czlonkowie) {
        ...
        const imie = safeStr(
          osoba.imie1 ?? osoba.imie ?? osoba.firstName ?? osoba.Imie
        )
        const nazwisko = safeStr(
          osoba.nazwisko ?? osoba.lastName ?? osoba.Nazwisko
        )
...
export async function enrichByNIP(nip: string): Promise<KRSEnrichment> {
  ...
  lines.push(`Branża TSL: ${result.branza_tsl ? 'TAK (kody PKD potwierdzają transport)' : 'NIE / NIEZNANE'}`)
